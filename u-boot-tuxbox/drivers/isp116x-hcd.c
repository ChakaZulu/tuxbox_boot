			      ...
			      int dir, void *buffer, int len)
{
	...
	int outlen ,done, stat, timeout, cc;
	int retries = 10;
	u8 * u8_buf = (u8*) buffer;
	...
	outlen =0;
	do {
		ptd[0].count = PTD_CC_MSK | PTD_ACTIVE_MSK |
			PTD_TOGGLE(usb_gettoggle(dev, epnum, dir_out));
		ptd[0].mps = PTD_MPS(max) | PTD_SPD(speed_low) | PTD_EP(epnum);
		ptd[0].len = PTD_LEN(max > len - done ? len - done : max) |
			PTD_DIR(dir);
		ptd[0].faddr = PTD_FA(usb_pipedevice(pipe));

		usb_dotoggle(dev, epnum, dir_out);
		done += PTD_GET_LEN(&ptd[0]);
		ptd[0].mps |= PTD_LAST_MSK;

		/* Pack data into FIFO ram */
		pack_fifo(isp116x, dev, pipe, ptd, 1, u8_buf, PTD_GET_LEN(&ptd[0]));
#ifdef EXTRA_DELAY
		wait_ms(EXTRA_DELAY);
#endif
		...
		cc = unpack_fifo(isp116x, dev, pipe, ptd, 1, u8_buf, PTD_GET_LEN(&ptd[0]));
		outlen += PTD_GET_LEN(&ptd[0]);
		if (PTD_GET_COUNT(&ptd[0]) < PTD_GET_LEN(&ptd[0]))
			break;

		u8_buf+=PTD_GET_LEN(&ptd[0]);
	} while (done < len);
