...
#define MEM_BASE		0x03900000
#else
#define MEM_BASE		0x01900000
#endif
...
#define MEM_SEG0_ADDR		(MEM_BASE+0x00000000)
#define MEM_SEG0_SIZE		1

#define MEM_SEG1_ADDR		(MEM_BASE+0x00200000)
#define MEM_SEG1_SIZE		1

#define MEM_SEG2_ADDR		(MEM_BASE+0x00400000)
#define MEM_SEG2_SIZE		2

#define MEM_SEG3_ADDR		(MEM_BASE+0x00800000)
#define MEM_SEG3_SIZE		0

#define MEM_SEG4_ADDR		(MEM_BASE+0x00900000)
#define MEM_SEG4_SIZE		0

#define MEM_SEG5_ADDR		(MEM_BASE+0x00a00000)
#define MEM_SEG5_SIZE		0

#define MEM_SEG6_ADDR		(MEM_BASE+0x00b00000)
#define MEM_SEG6_SIZE		0

#define MEM_SEG7_ADDR		(MEM_BASE+0x00c00000)
#define MEM_SEG7_SIZE		0
...
{
	unsigned long reg_mux;

	switch(mode)
	{
		case DENC_NTSC:
			mtdcr(DENC0_CR1,0x90000040);
			mtdcr(DENC1_CR1,0x90000040);
			break;
		case DENC_PAL:
			mtdcr(DENC0_CR1,0x91000040);
			mtdcr(DENC1_CR1,0x91000040);
			break;
	}

	reg_mux = mfdcr(DENCMUX);
	mtdcr(DENCMUX, reg_mux | 0x26000000);
...
extern int mpeg_showiframe_c(char * iframe,int len);
...
/* Sandali Start */
	s  = "edb toenv videofmt;";
#ifdef CFG_HUSH_PARSER
	run_command(s,0);
#else
	parse_string_outer(s, FLASH_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP);
#endif
/* Sandali End */
...
/*
	...
	s = "chpart 3; fsload 10 init.denc.uboot;";
#ifdef CFG_HUSH_PARSER
	if(run_command(s,0)<0)
	{
		denc_init(video_mode);
		puts("init.denc.uboot load fail\n");
	}
#else
	if(parse_string_outer(s, FLASH_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP) != 0)
	{
		denc_init(video_mode);
		puts("init.denc.uboot load fail\n");
	}
	...
	else
	{
		...
		{
			denc_init(video_mode);
			puts("init.denc.uboot exec fail\n");
		}
#else
		if(parse_string_outer(s, FLASH_PARSE_SEMICOLON | FLAG_EXIT_FROM_LOOP) != 0)
		{
...
#else
/*
#ifdef	CFG_VIDEO_STARTUP_IFRAME
extern int video_draw_startup_iframe( void );
